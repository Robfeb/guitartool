...
@Component({
  ...
  template: `
    <div class="flex flex-col gap-6">
      ...
      <!-- CAGED System Box -->
      <div class="bg-studio-dark border border-gray-800 rounded-lg p-4 md:p-6 shadow-xl" *ngIf="theory.showCagedPanel()">
        <h3 class="text-[10px] text-string font-bold uppercase tracking-widest mb-4">
          {{ lang.t('CAGED_SYSTEM') }}
        </h3>
        <app-caged-system></app-caged-system>
      </div>

      <!-- Songwriting Progressions Box -->
      <div class="bg-studio-dark border border-gray-800 rounded-lg p-4 md:p-6 shadow-xl" *ngIf="theory.showProgressionsPanel()">
        <h3 class="text-[10px] text-string font-bold uppercase tracking-widest mb-4">
          {{ lang.t('SONGWRITING') }}
        </h3>
